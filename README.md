# 雨课堂助手 Lite V2.1（Android）

这是从原桌面项目中独立出来的精简 Android 客户端，保留多节点、App 内登录、自动签到和自动答题。V2.1 修复二维码过期后旧连接不再响应的问题，并补充节点登录态即时校验和退出账号。仓库不包含任何可用 API Key，使用者需要自行配置 DeepSeek API。

## 功能

- 支持雨课堂主站、长江雨课堂、黄河雨课堂和荷塘雨课堂；各节点的登录状态相互隔离，切换后立即验证已有 session。
- 提供 240 秒限时微信扫码登录；每 60 秒关闭旧连接并建立新的登录 WebSocket，兼容顶层和嵌套 ticket；保留 App 内官方网页授权作为备用方式。
...
V1 已在 Android 16 真机验证安装、启动、工作台状态和 API 设置界面。V2.1 发布前会重新执行可用的本地、云端与设备检查；二维码扫码成功换取 session、各学校节点和真实课堂的签到/答题仍需配合对应账号与测试课程验证。桌面端已有协议为移植依据，但雨课堂内部接口随时可能变更。
